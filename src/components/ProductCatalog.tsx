...
export function ProductCatalog({
  ...
  values: Record<string, { unitPriceUsd?: string; isEditing?: boolean }>;
  onChangeValues: (
    code: string,
    patch: Partial<{ unitPriceUsd: string }>,
  ) => void;
    ...
    >
      {products.map((p) => (
        ...
        >
          ...
          <div className="mt-2">
            <div className="mb-1 text-[11px] text-slate-600">Precio (USD)</div>
            <input
              className={`w-full rounded-none border border-slate-300 px-2 py-1 text-xs outline-none focus:border-slate-500 ${
                values[p.code]?.isEditing
                  ? "bg-white text-slate-900"
                  : "bg-slate-50 text-slate-500"
              }`}
              inputMode="decimal"
              value={values[p.code]?.unitPriceUsd ?? ""}
              onChange={(e) => {
                const value = e.target.value;

                if (!/^\d*\.?\d*$/.test(value)) return;

                onChangeValues(p.code, { unitPriceUsd: value });
              }}
              placeholder="0.00"
              readOnly={!values[p.code]?.isEditing}
              disabled={!values[p.code]?.isEditing}
            />
          </div>
          ...
          <div className="mt-2 flex items-center justify-between gap-2">
            <button
              ...
              onClick={() => onToggleEdit(p.code, !values[p.code]?.isEditing)}
            >
              {values[p.code]?.isEditing ? "Guardar" : "Editar precio"}
            </button>
